...
class Environment:
    ...
    def show(self, agent_state, path=None):
        ...
        if path:
            if len(path) >= 2:
                for i in range(len(path) - 1):
                    line_start = (
                        int(path[i][0] * self.magnification), int((1 - path[i][1]) * self.magnification))
                    line_end = (int(path[i + 1][0] * self.magnification),
                                int((1 - path[i + 1][1]) * self.magnification))
                    cv2.line(self.image, line_start, line_end, (0, 150, 0), thickness=3)
        # Show the image
